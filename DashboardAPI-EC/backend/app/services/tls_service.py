...
def normalize(certificate: bytes, key: bytes, password: str, hostname: str) -> dict:
    ...
    try:
        if not key:
            ...
            private = (
                serialization.load_pem_private_key(key, secret)
                if b"-----BEGIN" in key
                else serialization.load_der_private_key(key, secret)
            )
        leaf = certs[0]
        ...
        raise ValueError(
            "Certificado/clave inválidos, contraseña incorrecta o clave que no corresponde."
        ) from None
